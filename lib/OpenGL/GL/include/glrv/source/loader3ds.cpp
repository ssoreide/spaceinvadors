...
#include <cmath>
#include <algorithm>
#include <glrv/include/loader3ds.h>

/** MODIFICADO por Roberto Vivo 2012 **/
...
void Model_3DS::Extent(float &xmin, float &xmax, float &ymin, float &ymax, float &zmin, float &zmax) const
{
    xmin=ymin=zmin=1000000000.0;
    xmax=ymax=zmax= -xmin;
    for(int i=0; i<Objects[0].numVerts; i++){
        xmin= min(xmin,Objects[0].Vertexes[i*3]);
        xmax= max(xmax, Objects[0].Vertexes[i*3]);
        ymin= min(ymin,Objects[0].Vertexes[i*3+1]);
        ymax= max(ymax, Objects[0].Vertexes[i*3+1]);
        zmin= min(zmin,Objects[0].Vertexes[i*3+2]);
        zmax= max(zmax, Objects[0].Vertexes[i*3+2]);
    };
}


void Model_3DS::LoadTexture(char *name){
    for(int i=0; i<numMaterials; i++){
        //Materials[i].tex.Load(name);
