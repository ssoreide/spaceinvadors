...
#define HOR_TAB_CHAR	0x09
#define CR_CHAR			0x0D
#define SPACE			0x20

#define LHR_IsSpace(c) ((HOR_TAB_CHAR == c) ||(CR_CHAR == c) || (SPACE == c))
...
protected:
	...
	virtual void Characters(const UGKS_String &rText, DWORD dwAppData, bool &bAbort)
	{
		UNUSED_ALWAYS(rText);
		UNUSED_ALWAYS(dwAppData);
		bAbort = false;
	...
	virtual void Comment(const UGKS_String &rComment, DWORD dwAppData, bool &bAbort)
	{
...
public:
	...
	UINT Read(UGKS_String lpszString);
...
protected:
	...
	virtual bool parseComment(UGKS_String &rComment);
	...
	char ReadChar(void)
	{
		...
		if (m_dwBufPos >= m_dwBufLen)
			return (NULL);
		return ParsedBuffer[m_dwBufPos++];
	}
...
protected:
	...
	/**
	 * STL string of characters being parsed
	 * @since 1.0
	 * @author Gurmeet S. Kochar
	 */
	UGKS_String	ParsedBuffer;
};
 ...
 * @author Gurmeet S. Kochar
 */
inline bool CLiteHTMLReader::parseComment(UGKS_String &rComment)
{
	ASSERT(!ParsedBuffer.empty());
	...
	if (ParsedBuffer.compare(m_dwBufPos, 4, "<!--"))
		return (false);
	//From this point, parsed buffer starts with a HTML comment
	UGKS_String lpszBegin = &ParsedBuffer[m_dwBufPos + 4];	
	// HTML comments end with two hyphen symbols '--'
	std::size_t lpszEnd = lpszBegin.find("--");

	// comment ending delimeter could not be found?
	if (std::string::npos == lpszEnd)
	...
	//At this point the string does not finissh with a comment. Pass only the comment string, not the rest

	UGKS_String	strComment = lpszBegin.substr(0, lpszEnd);
	...
	lpszEnd += 2; // (sizeof(TCHAR)* 2);
	while (LHR_IsSpace(lpszBegin[lpszEnd])) lpszEnd++;
									  ...
									  bool &bIsClosingTag)
{
	...
	UGKS_String auxString = &ParsedBuffer[m_dwBufPos];
	//LPCTSTR aux = UGKS_string2LPCTSTR(auxString);
	CString aux = UGKS_string2CString(auxString);

	UINT nRetVal = rTag.parseFromStr(aux,
